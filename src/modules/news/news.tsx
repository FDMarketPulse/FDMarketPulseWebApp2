...
const News = () => {
  ...
  const isNewsSentimentLoading = useRootSelector(
    ChatSel.isNewsSentimentLoading
  );
  ...
  useEffect(() => {
    if (newsContent.content && newsContent.content.length > 0) {
              ...
              <Card
                title={
                  <Row gutter={32}>
                    ...
                    <Col xs={24} sm={24} md={8} lg={8} xl={8}>
                      {formatDistanceToNow(new Date(Number(item.date) * 1000), {
                        addSuffix: true,
                      })}
                    </Col>
                  </Row>
